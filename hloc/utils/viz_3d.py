...
def plot_cameras(
    ...
    for image_id, image in reconstruction.images.items():
        vertices = plot_camera_colmap(
            fig, image, reconstruction.cameras[image.camera_id], **kwargs)
        image_vertices = ImageVertices()
        image_vertices.IMAGE_ID = image_id
        image_vertices.vertices = vertices
        image_vertices_list.append(image_vertices)

    return image_vertices_list


def plot_reconstruction(
        fig: go.Figure,
        rec: pycolmap.Reconstruction,
        max_reproj_error: float = 2.0,#6 xujx update
        color: str = 'rgb(0, 0, 255)',
        name: Optional[str] = None,
    ...
    if cameras:
        image_vertices_list = plot_cameras(fig, rec, color=color, legendgroup=name, size=cs)
    return image_vertices_list
