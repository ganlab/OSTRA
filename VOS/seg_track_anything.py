...
aot_model2ckpt = {
    "deaotb": "./checkpoints/DeAOTB_PRE_YTB_DAV.pth",
    "deaotl": "./checkpoints/DeAOTL_PRE_YTB_DAV",
    "r50_deaotl": "./checkpoints/R50_DeAOTL_PRE_YTB_DAV.pth",
}
...
def tracking_objects_in_video(SegTracker, input_video):
    ...
    tracking_result_dir = f'{os.path.join(os.path.dirname(__file__), "result", f"{video_name}")}'
    create_dir(tracking_result_dir)
    
    io_args = {
        'tracking_result_dir': tracking_result_dir,
        'output_mask_dir': f'{tracking_result_dir}/images-mask',
        'output_masked_frame_dir': f'{tracking_result_dir}/{video_name}_masked_frames',
        'output_video': f'{tracking_result_dir}/videos/{video_name}_seg.mp4', # keep same format as input video
...
def video_type_input_tracking(SegTracker, input_video, io_args, video_name):
    ...
    result_dir = io_args['tracking_result_dir']
    video_path = os.path.join(result_dir, 'videos')
    ...
    with torch.cuda.amp.autocast():
        while cap.isOpened():
            ...
            if frame_idx == 0:
                ...
                save_prediction(new_obj_mask, output_mask_dir, str(frame_idx + 1) + '_new.png')
            ...
            save_prediction(pred_mask, output_mask_dir, str(frame_idx + 1) + '.png')
    ...
    # imageio.mimsave(io_args['output_gif'], masked_pred_list, duration=(1000*1/fps))#update xujx
