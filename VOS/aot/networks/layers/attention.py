...
class LocalGatedPropagation(nn.Module):
    ...
    def forward(self, q, k, v, u, size_2d):
        ...
        q = q.view(-1, self.d_att, h, w).to(torch.float16)
        k = k.view(-1, self.d_att, h, w).to(torch.float16)
        v = v.view(-1, self.num_head, hidden_dim, h * w).to(torch.float16)
        ...
        global_attn = self.local2global(local_attn, h, w).to(torch.float16)
